...
extension Manifest {
    ...
    /// 
    /// - Parameters:
    ///   - customImportModuleNames: Names of any modules to import (would commonly contain custom product type definitions).
    ///   - customProductTypeHandler: Closure that will be called once for each custom product type in the manifest; it should return a SourceCodeFragment for the product type.
    /// 
    /// Returns: a string containing the full source code for the manifest.
    public func generateManifestFileContents(
        customImportModuleNames: [String] = [],
        customProductTypeHandler: ManifestSourceGenerationCustomProductHandler? = nil
    ) rethrows -> String {
        // Generate the source code fragment for the top level of the package
        // expression.
        let packageExprFragment = try SourceCodeFragment(from: self, customProductTypeHandler: customProductTypeHandler)
        ...
        return """
            ...
            \(customImportModuleNames.map{ "import \($0)\n" }.joined())
    ...
    public var generatedManifestFileContents: String {
        return self.generateManifestFileContents(customProductTypeHandler: nil)
    }
}
...
public typealias ManifestSourceGenerationCustomProductHandler = (_ typeName: String, _ propertyData: Data) throws -> SourceCodeFragment?
...
fileprivate extension SourceCodeFragment {
    ...
    init(from manifest: Manifest, customProductTypeHandler: ManifestSourceGenerationCustomProductHandler?) rethrows {
        ...
        if !manifest.products.isEmpty {
            let nodes = try manifest.products.map{ try SourceCodeFragment(from: $0, customProductTypeHandler: customProductTypeHandler) }
    ...
    init(from product: ProductDescription, customProductTypeHandler: ManifestSourceGenerationCustomProductHandler?) rethrows {
        var params: [SourceCodeFragment] = []
        ...
        case .custom(let customType, let propertyData):
            if let subnode = try customProductTypeHandler?(customType, propertyData) {
                self = subnode
            }
            else {
